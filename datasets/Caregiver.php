...
// Common basis
$NAMES = array(0 => array("Maria", 'F'), 1 => array("Tiago", 'M'), 2 => array("Carla", 'F'), 3 => array("Jorge", 'M'));
...
function create_care_givers() {
  ...
  for ($i=0; $i < 20; $i++) {
    ...
    for ($j=0; $j < $nreviews; $j++) {
      $ireviews[$j] = $COMMENTZ[$new_r];
      array_push($repeat,$new_r);
      if($new_r >= count($COMMENTZ)) {
        $new_r=0;
      } else $new_r++;
    }
